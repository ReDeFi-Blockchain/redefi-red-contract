...
2. Init project with `npm install`.
3. Deploy BatchSender contract. `npx hardhat run scripts/deployBatchSender.ts`
4. Set `TOKEN_ADDRESS` and `BATCH_CONTRACT_ADDRESS` in .env file.
5. Approve tokens to be used by BatchSender. `npx hardhat run scripts/approveTokensForBatchContract.ts`
6. Update target gas price `MAXIMUM_GAS_PRICE` in .env file.
7. Start sending tokens to recepients `npx hardhat run scripts/sendTokenBatches.ts`